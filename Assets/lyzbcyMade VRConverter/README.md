...
- 窗口顶部的状态横幅会实时展示目标平台、当前步骤和依赖检测结果，颜色变化用于提示风险。
- 三个操作卡片（执行全部 / 步骤一 / 步骤二）采用图标+说明 + 主按钮的组合，执行状态与日志位置清晰。
- 底部执行日志与提示区域保持与其他插件一致的卡片语言，适合截图到团队文档。
- 所有关键信息（XR 依赖、平台切换、Rig 结果）都以可视化标签呈现，整个流程更友好。
